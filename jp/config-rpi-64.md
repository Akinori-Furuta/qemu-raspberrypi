...
current directory は _/PathTo/RpiVMFiles_ です。[QEMU で実行する Rasiberry Pi イメージファイルをスクリプトを使って作る](rpi-image-script.md) の続きの作業です。
...
[`./rpi3vm64-1st.sh`](../downloads/host/rpi3vm64-1st.sh) を実行して 30 秒 ～ 2 分程度経過すると次の様な表示で止まったように見えます。空き領域の trim 処理をしています。20 ～ 30 分程度掛かります。
...
<a name="rpi3vm64-conf"></a>

## (参考) rpi3vm64.conf, rpi3vm64-vnc.conf を設定する

[`rpi3vm64.conf`](../downloads/host/rpi3vm64.conf) と [`rpi3vm64-vnc.conf`](../downloads/host/rpi3vm64-vnc.conf) は qemu-system-aarch64 コマンドに渡すパラメータを設定するファイルです。修正しなくてもある程度動作する様にしてあります。自動的なパラメータ設定で上手くいかない場合は修正して下さい。

> [!TIP]
> qemu 起動スクリプトが rpi3vm64-vnc.sh の場合は拡張子 .sh の部分を .conf に置き換えたファイル rpi3vm64-vnc.conf が読み込まれます。

|shell variable|default value|description|
|--------------|-------------|-----------|
|KernelFile|bootfs/kernel8.img|-kernel オプションに渡すパラメータです。kernel image file を指定します。|
|InitrdFile|bootfs/initramfs8|-initrd オプションに渡すパラメータです。initrd image file を指定します。<sup>*1</sup>|
|DtbFile|bootfs/bcm2710-rpi-3-b-qemu.dtb|-dtb オプションに渡すパラメータです。Device Tree Blob file を指定します。|
...
カレントディレクトリが _/PathTo/RpiVMFiles_ だった場合の例をいくつか示します。rpi3vm64.sh をリネームした場合の参考にして下さい。
