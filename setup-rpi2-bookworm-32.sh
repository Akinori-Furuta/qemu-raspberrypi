...
for f in "downloads/host/rpi2vm32-1st.conf.bookworm" \
	 "downloads/host/rpi2vm32-2nd.conf.bookworm"
do
	if [[ ( ! -h "${f}" ) && ( ! -f "${f}" ) ]]
	then
...
for f in "downloads/host/rpi2vm32-1st.conf" \
	 "downloads/host/rpi2vm32-2nd.conf" \
	 "downloads/host/rpi2vm32.conf" \
...
echo "$0: HELP: Imager:      ./rpi2image.sh {find | /dev/sdX}"
echo "$0: HELP:   Copy image from bootable media."
echo "$0: HELP: "
